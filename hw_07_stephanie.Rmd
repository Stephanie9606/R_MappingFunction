...
df %>% 
  mutate(data = map(data, ~pivot_longer(., cols = c(contains("/")),
             names_to = "Date",
             # names_ptypes = list(Date = integer()), # need revise
             values_to = "Cases"))) ->
  df_long
```
...
anti_join(df_all, df_pop, by = c("Country_State" = "Location")) %>% 
...
```{r}
anti_join(df_pop, df_all, by = c("Location" = "Country_State")) %>% 
  distinct(Location) %>% 
  nrow()
```
...
```{r}
left_join(df_pop, df_all, by = c("Location" = "Country_State")) ->
  df_pop
```
...
# Poptol_Rank
df_pop %>% 
  distinct(Location, PopTotal) %>% 
  mutate(Poptol_Rank = min_rank(-PopTotal)) %>% 
  full_join(df_pop, by = c("Location", "PopTotal")) ->
  df_pop
# Popdes_Rank
df_pop %>% 
  distinct(Location, PopDensity) %>% 
  mutate(Popdes_Rank = min_rank(-PopDensity)) %>% 
  full_join(df_pop, by = c("Location", "PopDensity")) ->
  df_pop
# show the top 10 countries for pop and dens
# pops
df_pop %>% 
  select(Location, Poptol_Rank) %>% 
  distinct() %>% 
  arrange(Poptol_Rank, n = 10)
# dens
df_pop %>% 
  select(Location, Popdes_Rank) %>% 
  distinct() %>% 
  arrange(Popdes_Rank, n = 10)
```
